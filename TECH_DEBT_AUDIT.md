...
v10.0 以降、Wave 1/2（OP-024/060/061/067/069）・UI 改修・OP-013 E2E・Vault 正本化が進み、**前回 Top 5 の大半は解消または降格**した。一方で、**セキュリティ経路の Fail-Open 残存**（Immune System）と **Public Beta 後に残る構造負債**（JobQueue 契約・Error 階層・依存 ignore・App.tsx）が次の主戦場である。
...
1. **Immune System 評価失敗時の Fail-Open**（`tool_call_router.rs` / `stream.rs`）— OP-024 課金 Fail-Closed と対照的  
2. **commerce webhook の KC 付与 / SSE 握り潰しの一貫性欠如**  
3. **`scc` RUSTSEC-2026-0205**（`serial_test` 経由・allowed warning）  
4. **Stripe キー名混在**（api-server=`STRIPE_API_KEY` vs MCP/UI=`STRIPE_SECRET_KEY`）  
5. **App.tsx 786 行 god-shell**（lazy 29 個でもシェル責務未分離）
...
| **P4** | **OP-068 / OP-030–034** | 🟡 | Upstream | 継続 | 継続 |
| **P5** | **App.tsx god-shell**（HEX は OP-029 解消） | 🟡 | App.tsx 分割は未着手 | 4h | HEX `[RESOLVED]` / shell 継続 |
...
| 次元 | 指摘 | 対象 | 深刻度 | 工数 | Status |
|---|---|---|---|---|---|
| **Dim 1** | App.tsx god-shell（786 行・lazy 29） | `apps/management-console/src/App.tsx` | 🟡 | 4h | `[NEW]` |
...
| **Dim 5** | `.cargo/audit.toml` ignore 多数（OP-030–034 / OP-068） | `.cargo/audit.toml:12-62` | 🟡 | Upstream | 継続 |
...
直近の市場接触ゲート（`near_term_public_beta_plan.md` v5.1）は **Human 作業（NT-1 Vault 等）が律速**。P1（OP-075）/ QW-19..23 / OP-054 可視性は **2026-07-10 実装済**。残る主戦場は App.tsx 分割・OP-051 コード実装（ADR-054 承認後）・OP-068 Upstream・OP-075-B。

---

*Generated by `/tech-debt-audit` workflow — 2026-07-10 v11.0（Wave 1/2 完了後差分・ホットスポット再計測・12次元再スキャン）*
