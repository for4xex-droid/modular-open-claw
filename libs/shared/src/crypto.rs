...
pub fn derive_biome_key(hub_secret: &str) -> Result<[u8; 32], String> {
...
mod tests {
    ...
    fn test_derive_biome_key_consistency() {
        ...
        let key1 = derive_biome_key(secret).unwrap();
        let key2 = derive_biome_key(secret).unwrap();
        assert_eq!(key1, key2);
    ...
    fn test_derive_biome_key_uniqueness() {
        let key1 = derive_biome_key("secret-a").unwrap();
        let key2 = derive_biome_key("secret-b").unwrap();
        assert_ne!(key1, key2);
    }
}
