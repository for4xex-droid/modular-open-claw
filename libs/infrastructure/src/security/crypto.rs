...
pub fn encrypt_aes256gcm(
    ...
    let cipher = Aes256Gcm::new_from_slice(key)
        .map_err(|e| AiomeError::SecurityViolation { reason: format!("Failed to initialize cipher: {:?}", e) })?;

    let nonce_bytes: [u8; 12] = rand::thread_rng().gen();
    let nonce = Nonce::from_slice(&nonce_bytes);

    let ciphertext = cipher.encrypt(nonce, data)
        .map_err(|e| AiomeError::SecurityViolation { reason: format!("Encryption failed: {:?}", e) })?;
...
pub fn decrypt_aes256gcm(
    ...
    cipher.decrypt(nonce, ciphertext)
        .map_err(|e| AiomeError::SecurityViolation { reason: format!("Decryption failed: {:?}", e) })
}
...
mod tests {
    ...
    fn test_encrypt_nonce_uniqueness() {
        ...
        assert_ne!(encrypted1, encrypted2, "Encrypted payloads should differ due to unique nonces");
