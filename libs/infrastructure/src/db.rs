...
impl DatabasePool {
    ...
    /// In SQLite: (julianday('now') - julianday(col)) * 24 * 60 > minutes
    /// In Postgres: col < NOW() - INTERVAL 'x minutes'
    ...
    /// Generates UPSERT query (INSERT OR REPLACE for SQLite, ON CONFLICT for Postgres)
    pub fn upsert_query(
        &self,
        table: &str,
        conflict_col: &str,
