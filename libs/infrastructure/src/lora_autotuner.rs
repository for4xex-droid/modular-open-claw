...
impl LoraAutotuner {
    ...
    pub fn suggest_hyperparams(metrics: &TrainingMetrics) -> TunedHyperparams {
        ...
        let recent_variance: f64 = metrics.loss_history.iter().skip(n - 3).map(|&x| (x - l_last).abs()).sum();
...
mod tests {
    ...
    fn test_suggest_hyperparams_stagnation() {
        ...
        assert!(new_params.learning_rate > 1e-4, "LR should increase for stagnation");
    }
    ...
    fn test_suggest_hyperparams_oscillation() {
        ...
        let new_params = LoraAutotuner::suggest_hyperparams(&metrics);
        assert!(new_params.learning_rate < 5e-4, "LR should decrease for oscillation");
    }
    ...
    fn test_suggest_hyperparams_overfitting() {
        ...
        let new_params = LoraAutotuner::suggest_hyperparams(&metrics);
        assert!(new_params.lora_rank < 16, "Rank should decrease to prevent overfitting");
        assert!(new_params.epochs < 5, "Epochs should decrease to prevent overfitting");
    }
}
