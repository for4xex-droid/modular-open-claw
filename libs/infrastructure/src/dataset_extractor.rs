...
impl DatasetExtractor {
    ...
    pub async fn extract_to_jsonl<T: SoulStore + ?Sized>(
        ...
        let soul_json = match soul_data {
            Some(v) => v,
            None => return Err(AiomeError::Infrastructure {
        ...
        // フォルダ作成
        tokio::fs::create_dir_all(&self.output_dir).await.map_err(|e| AiomeError::Infrastructure {
        ...
        let mut file = tokio::fs::File::create(&output_file).await.map_err(|e| AiomeError::Infrastructure {
        ...
        for exp in experiences {
            let role = exp.get("role").and_then(|r| r.as_str()).unwrap_or("unknown");
        ...
        let line_str = serde_json::to_string(&jsonl_line).map_err(|e| AiomeError::Infrastructure {
            reason: format!("JSONL serialization failed: {}", e),
        })?;
        
        file.write_all(format!("{}\n", line_str).as_bytes()).await.map_err(|e| AiomeError::Infrastructure {
...
mod tests {
    ...
    async fn test_extract_to_jsonl_success() {
        ...
        // Act
        let result = extractor.extract_to_jsonl(&*store, "test_soul_123", "job_123").await;
        ...
        assert_eq!(lines.len(), 1, "Should contain exactly 1 line representing the whole conversation block");
    ...
    async fn test_extract_to_jsonl_empty() {
        ...
        // Act
        let result = extractor.extract_to_jsonl(&*store, "test_soul_444", "job_444").await;

        // Assert
        assert!(result.is_err(), "Extraction should fail if no data available");
