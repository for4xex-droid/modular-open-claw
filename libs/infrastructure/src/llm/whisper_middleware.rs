...
#[async_trait]
impl<A: SoulDomainAdapter + 'static, E: SamsaraEngine + Send + Sync + 'static> SoulMiddleware<A, E> for WhisperMiddleware<A, E> {
    async fn process(
        &self,
...
mod tests {
    ...
    impl SoulDomainAdapter for DummyAdapter {
        ...
        fn execute_defense<'a>(&'a self, _: &'a soul::defense::DefenseAction, _: &'a str) -> Pin<Box<dyn Future<Output = Result<(), SoulError>> + Send + 'a>> { Box::pin(async { Ok(()) }) }
    ...
    async fn test_whisper_middleware_appends_thought() {
        ...
        assert!(ctx.experience.content.contains("Whisper:"), "Should contain whisper thought");
    }
}
