...
impl CostCircuitBreaker {
    ...
    pub async fn check_state(&self) -> Result<CostStatus, AiomeError> {
        ...
        // バイパススイッチ（手動拡張）の確認
        let bypass_amount = self.jq.get_setting_value("cost_bypass_amount")
            .await
            .ok()
            .flatten()
            .and_then(|s| s.parse::<f64>().ok())
        ...
        if final_tripped {
            warn!("🚨 [CostCircuitBreaker] TRIP! 24h cost ${:.4} exceeds limit ${:.4}", total_usd, limit_usd + bypass_amount);
        }
...
impl CostBypassSwitch {
    ...
    pub async fn expand_quota(&self, amount_usd: f64) -> Result<(), AiomeError> {
        let current = self.jq.get_setting_value("cost_bypass_amount")
            .await
            .ok()
            .flatten()
            .and_then(|s| s.parse::<f64>().ok())
        ...
        self.jq.update_setting("cost_bypass_amount", &(current + amount_usd).to_string(), "system", false).await?;
        info!("✅ [CostBypassSwitch] Quota expanded by ${:.2}.", amount_usd);
        Ok(())
    }
}
