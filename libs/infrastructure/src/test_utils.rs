...
pub mod mock_job_queue {
    use aiome_contracts::contracts::{ArenaMatch, ImmuneRule, KarmaEntry, OracleVerdict, SamsaraEvent, FederatedMetrics};
    use aiome_contracts::traits::{Job, JobStatus, JobQueue, KarmaSearchResult, SnsMetricsRecord, Expression, SoulStore};
    ...
    impl JobQueue for MockJobQueue {
        ...
        async fn fetch_job(&self, id: &str) -> Result<Option<Job>, AiomeError> { Ok(Some(Job { id: id.to_string(), category: "mock".into(), topic: "mock".into(), style: "mock".into(), priority: 0, status: JobStatus::Pending, created_at: "".into(), updated_at: "".into(), started_at: None, last_heartbeat: None, tech_karma_extracted: false, execution_log: None, creative_rating: None, karma_directives: None, permission_manifest: None, agent_id: None, error_message: None, sns_platform: None, sns_content_id: None, published_at: None, output_artifacts: None })) }
        ...
        async fn get_system_agent_id(&self) -> Result<Uuid, AiomeError> { Ok(Uuid::nil()) }
    ...
    impl LlmProvider for MockLlm {
        async fn complete(&self, _: &str, _: Option<&str>) -> Result<LlmResponse, AiomeError> { unimplemented!() }
        async fn test_connection(&self) -> Result<(), AiomeError> { Ok(()) }
        fn name(&self) -> &str { "mock" }
        async fn complete_with_cache(&self, _: aiome_contracts::llm::LlmRequest) -> Result<LlmResponse, AiomeError> { unimplemented!() }
    }
}
