...
use aiome_contracts::traits::{
    Job, JobQueue, JobStatus, KarmaSearchResult, SnsMetricsRecord,
};
use aiome_contracts::contracts::{
    ArenaMatch, FederatedKarma, ImmuneRule, SamsaraEvent, OracleVerdict,
};
...
impl JobQueue for MockJobQueue {
    ...
    async fn get_agent_stats(&self) -> Result<AgentStats, AiomeError> {
        Ok(AgentStats { level: 1, exp: 0, resonance: 0, creativity: 0, fatigue: 0 })
    }
    ...
    async fn fetch_relevant_karma(&self, _: &str, _: &str, _: i64, _: &str) -> Result<KarmaSearchResult, AiomeError> {
        Ok(KarmaSearchResult { entries: vec![], is_ood: false, max_score: 0.0 })
    }
    ...
    async fn get_system_agent_id(&self) -> Result<Uuid, AiomeError> { Ok(Uuid::new_v4()) }
}
