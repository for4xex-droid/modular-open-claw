...
mod tests {
    use super::*;
    use aiome_contracts::contracts::{ArenaMatch, KarmaEntry, ImmuneRule, OracleVerdict, SamsaraEvent, FederatedMetrics};
    use aiome_contracts::traits::{Job, JobStatus, KarmaSearchResult, SnsMetricsRecord, Expression, SoulStore};
    ...
    impl JobQueue for MockJobQueue {
        ...
        async fn enqueue(&self, _: &str, _: &str, _: &str, _: Option<&str>, _: Option<aiome_contracts::security::PermissionManifest>, _: Option<Uuid>, _: i32) -> Result<String, AiomeError> { Ok("mock".into()) }
        async fn dequeue(&self, _: &[&str]) -> Result<Option<Job>, AiomeError> { Ok(None) }
        async fn fetch_job(&self, id: &str) -> Result<Option<Job>, AiomeError> { Ok(None) }
        ...
        async fn fetch_all_karma(&self, _l: i64) -> Result<Vec<serde_json::Value>, AiomeError> { Ok(self.karma_lessons.iter().map(|l| json!({"lesson": l})).collect()) }
    ...
    impl SoulStore for MockJobQueue {
        async fn load_soul(&self, _: &str) -> Result<Option<serde_json::Value>, AiomeError> { Ok(None) }
        async fn store_soul_fragment(&self, _: &str, _: &str) -> Result<(), AiomeError> { Ok(()) }
        async fn fetch_latest_soul_fragment(&self) -> Result<Option<(String, String)>, AiomeError> { Ok(None) }
    }
    ...
    async fn test_soul_transmutation() {
        ...
        let llm = Arc::new(MockLlm { mutation_response: "X Y Z".to_string() });
