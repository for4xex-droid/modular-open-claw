...
pub trait SecurityOps {
    async fn do_get_security_request_count(&self, agent_id: Option<Uuid>) -> Result<u32, AiomeError>;
    async fn do_increment_security_request_count(&self, agent_id: Option<Uuid>) -> Result<u32, AiomeError>;
}

#[async_trait]
impl SecurityOps for UniversalJobQueue {
    async fn do_get_security_request_count(&self, agent_id: Option<Uuid>) -> Result<u32, AiomeError> {
        let id_str = agent_id.map(|u| u.to_string()).unwrap_or_else(|| "SYSTEM".to_string());
        
        match &self.pool {
            DatabasePool::Sqlite(p) => {
                let row = sqlx::query("SELECT request_count FROM security_audit WHERE agent_id = ?")
                    .bind(&id_str)
                    .fetch_optional(p)
                    .await
            ...
            DatabasePool::Postgres(p) => {
                let row = sqlx::query("SELECT request_count FROM security_audit WHERE agent_id = $1")
                    .bind(&id_str)
                    .fetch_optional(p)
                    .await
                    .map_err(|e| AiomeError::Infrastructure { reason: e.to_string() })?;
