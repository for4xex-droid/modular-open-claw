...
mod tests {
    ...
    async fn test_progressive_skill_mtime_invalidation() {
        use infrastructure::skills::{WasmSkillManager, VerifiedSkill};
    ...
    fn test_virtual_path_resolution() {
        ...
        let resolved = sandbox.resolve_virtual_path("/mnt/workspace/test.txt").unwrap();
    ...
    async fn test_soul_middleware_structure() {
        use soul::pipeline::{SoulMiddleware, SoulContext};
