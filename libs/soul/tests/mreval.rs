...
use soul::pipeline::{SoulPipeline, SoulContext, SoulMiddleware};
...
impl SoulDomainAdapter for MREvalAdapter {
    ...
    fn execute_defense<'a>(&'a self, _: &'a DefenseAction, _: &'a str) -> Pin<Box<dyn Future<Output = Result<(), SoulError>> + Send + 'a>> { Box::pin(async { Ok(()) }) }
...
impl SamsaraEngine for MREvalEngine {
    ...
    fn rebirth<'a>(&'a self, s: AgentSoul) -> Pin<Box<dyn Future<Output = Result<AgentSoul, SoulError>> + Send + 'a>> { Box::pin(async { Ok(s) }) }
    fn distill<'a>(&'a self, _: &'a AgentSoul) -> Pin<Box<dyn Future<Output = Result<soul::instinct::Instinct, SoulError>> + Send + 'a>> { Box::pin(async { Ok(Default::default()) }) }
    fn dream<'a>(&'a self, s: AgentSoul) -> Pin<Box<dyn Future<Output = Result<AgentSoul, SoulError>> + Send + 'a>> { Box::pin(async { Ok(s) }) }
}
...
async fn test_mreval_full_pipeline() {
    ...
    soul.persona_boundaries.forbidden_topics.push("modern technology".to_string());
    soul.persona_boundaries.constraints.push("Talk like a medieval knight".to_string());
    ...
    impl soul::pipeline::SoulMiddlewareNext<MREvalAdapter, MREvalEngine> for MockNext {
        fn run<'a, 'b>(&'a self, _: &'b mut SoulContext<'_, MREvalAdapter, MREvalEngine>) -> Pin<Box<dyn Future<Output = Result<(), SoulError>> + Send + 'b>> where 'a: 'b { Box::pin(async { Ok(()) }) }
    }
    
    guard.process(&mut ctx_a, &MockNext).await.unwrap();
    assert!(ctx_a.is_rejected, "MREval: Bounding should reject forbidden modern technology topic");
    ...
    assert_eq!(ctx_b.recalled_experiences.len(), 1, "MREval: Recalling should find 1 relevant experience");
    assert!(ctx_b.recalled_experiences[0].content.contains("Iron Gate"), "MREval: Recalling should retrieve the Battle at Iron Gate");
    ...
    assert_eq!(ctx_c.recalled_experiences.len(), 1, "MREval: Recalling should find 1 relevant semantic summary");
    assert!(ctx_c.recalled_experiences[0].content.contains("Dragon War"), "MREval: Recalling should retrieve the Dragon War insight");
