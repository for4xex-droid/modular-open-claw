...
- **OP-093**: SSRF / robots.txt / Hook pre·post 拒否にも安定 `reason_code=` を付与。`mcp_validate_denied` / hook deny アサート強化。
...
- **Stripe Price ID 確定（2026-07-05）**: Pro $19.99/月 → `price_1TpXFpBcUTwo5TwLmK9SQbKL`（Payment Link `plink_1TpXHCBcUTwo5TwLnO1BJneY`）。ローカル `.env` 反映済み。**OP-057-R（本番 env・決済→Pro 自動有効化）は Biome ブラッシュアップと同一バッチで後回し**（2026-07-05 ユーザー決定）。
...
- **WebGL / Canvas テーマカラー同期 (U-002) 改善**:
  - `apps/management-console/src/lib/biome/BiomeCellGrid.tsx` [MODIFY]: `ThemeBridge` の import 順序をファイル先頭へ修正。また、モジュールスコープの静的 `ELEMENT_COLORS` を廃止し、`computeElementColor` 内で動的に `ThemeBridge.getElementColor()` を参照することでテーマ切替時の追従性を 100% 達成するよう追加修正。
...
- **WASMデータ・パッキング精度崩壊の修正とビジュアル品質革命の実装 (Phase 8 実装完了)**:
  - WASMエンジンからの要素値転送時のパッキング方式（float32でのu16値2組パッキング）による精度崩壊を解消するため、3番目のデータテクスチャ（`gridTex2`）を追加し、すべての元素値（C, N, P, H, O, S, Fe, Si）をそのままf32で転送する完全分離データパイプラインを構築。
  - セル表現の美化と3Dジェル感向上のため、`grid.frag`に擬似法線計算およびPhongライティング（diffuse+specular）を実装。
  - セル中央に輝点（細胞核）および外周に薄いリング（細胞膜）を重ねる微細構造を追加。
  - 視線角度に応じたフレネルリム発光効果（端が青く光る半透明感）およびメタボール境界の柔らかなグローを実装。
  - 形態別（Basic, Producer, Consumer, Predator, Decomposer）の内部模様コントラストを強化し、形態ごとに固有の色味（Predator=赤み、Producer=緑み、Consumer=暖色、Decomposer=紫など）を適用する色相・彩度補正を追加。
  - WebGL2モックを拡張し、追加したテクスチャユニットバインド（TEXTURE4）を検証するユニットテストをTDDサイクルにて正常通過。

### Added
- **細胞形態の多様性革命 (Phase 8)**:
  - インスタンス描画を廃止し、フルスクリーン Quad + 2枚のデータテクスチャ（RGBA32F、128×128）を用いた描画パイプラインへ完全移行。毎フレーム `renderView` データをテクスチャ形式にパックして GPU へ転送することで、データ転送帯域幅を 50% 削減（1,024 KB → 512 KB）し描画効率を劇的に向上。
  - `grid.frag` を全面的に書き換え、3x3近傍のセルに対する 1/(d²+0.15) 距離場を合成するメタボール（融合細胞）アルゴリズムを導入。これにより、従来の無機質な水玉模様から、隣接細胞同士が有機的に融合するアメーバ状のセルクラスタ表示を実現。
  - 円周平均法を用いた HSL 色空間カラー生成を実装。8元素の比率を色相（Hue）、偏り具合を彩度（Saturation）、元素総量を明度（Lightness）に対応させ、美しくなめらかな色彩表現を実現。
  - セル形態（Basic, Producer, Consumer, Predator, Decomposer）に応じた局所的な内部模様（ドット、同心円の脈動、波状、放射状スパイク、渦巻き）を描画。
  - メタボール場の勾配から輪郭を検出するエッジグロー効果、および隣接セル間のエネルギー勾配を利用したエネルギーフロー（光の筋が流れるアニメーション）を実装。
  - 非アクティブ領域に 2D ボロノイ（Cellular）ノイズを用いた有機的なサイバー背景演出を追加。

### Fixed
- **インスタンス描画関連リソースのクリーンアップ**:
  - `BiomeRenderer.tsx` からインスタンス用 VBO および属性の Divisor 設定、`gl.drawArraysInstanced` などの冗長なコードを削除し、WebGLリソースリークを防ぐための破棄処理を実装。

- **Biome シミュレーションゲームのUX及びビジュアル品質の大幅向上 (Phase 1-5)**:
  - インタラクティブなキャンバス操作を実装。クリック/ドラッグによる元素注入、キャンバスホバーでのセル詳細（生存状態、形態、エネルギー、C/N/P/H比率）を表示するグラスモーフィズムツールチップ、および災害（隕石/氷河期）地点のクリック指定に対応。
  - WebGL2ベースのビジュアル演出を強化。輝度抽出と2パス双方向ブラーを適用するBloomポストプロセスシェーダー `bloom.frag` の新規実装、形態別カラーリングを適用したサイバーグリッド背景 `grid.frag` / `grid.vert` の更新、およびマイルストーン（50, 100, 150世代）時の画面フラッシュ演出を実装。
  - 5ステップのスポットライト対話型チュートリアル `BiomeTutorial.tsx` を新規実装。初回起動時（`localStorage` 保存値がない場合）の自動表示、およびコントロールパネルの「❓ 遊び方」ボタンからの手動表示に対応。
  - 別ウインドウ（ポップアップ）でのゲームプレイに対応。`biome-popup.html` および専用のエントリーポイント `biome-popup-entry.tsx` を作成し、`vite.config.ts` をマルチエントリービルド構成（Rollup input）に拡張。
  - ホーム画面（`HomePage.tsx`）の左側サイドバーへ、グラスモーフィズムデザインの「🎮 バイオーム」クイック起動カードを追加。ここからワンクリックでタブ切り替えまたは別ウインドウでのポップアップ起動が可能。
  - すべての UI 表示テキストを完全日本語化（テスト用の `aria-label` は英語を維持して互換性を担保）。

### Fixed
- **BiomeGame における React フック順序違反の修正**:
  - `BiomeGame.tsx` 内で早期ローディング return の後ろで宣言されていた `useCallback` （座標クリック、ホバーコールバック）を return の前に再配置し、レンダリング間でフック呼び出し数が変わる React フックのルール違反エラー（`Rendered more hooks than during the previous render`）を解決。
- **gridCoords テスト環境の Jest 適合化**:
  - `gridCoords.test.ts` に残存していた `vitest` のインポートを削除し、Jest テストランナーの下で正しくグローバル expect を利用してパスするように修正。
