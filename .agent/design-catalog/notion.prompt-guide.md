...
詳細は `notion.design.md` を参照（オンデマンド）。

## Key Colors

| Role | HEX |
|------|-----|
| Notion Blue (CTA) | `#0075de` |
| Active Blue (pressed) | `#005bab` |
| Pure White (background) | `#ffffff` |
| Warm White (alt bg) | `#f6f5f4` |
| Near-Black (text) | `rgba(0,0,0,0.95)` |
| Warm Gray 500 (secondary) | `#615d59` |
| Warm Gray 300 (muted) | `#a39e98` |
| Warm Dark | `#31302e` |
| Deep Navy (brand) | `#213183` |
| Focus Blue | `#097fe8` |
| Badge Blue Bg / Text | `#f2f9ff` / `#097fe8` |
| Whisper Border | `1px solid rgba(0,0,0,0.1)` |

## Typography Essentials

- **Primary**: `NotionInter` (fallback: Inter, system-ui) — OpenType `"lnum"`, `"locl"` on display/headings
- **Weights**: 400 (body), 500 (UI), 600 (emphasis/nav), 700 (headings/display)
- **Display tracking**: -2.125px at 64px, -1.875px at 54px, -0.625px at 26px, normal at 16px
- **Badge tracking**: +0.125px at 12px (only positive tracking in system)
