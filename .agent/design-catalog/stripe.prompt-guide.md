...
詳細は `stripe.design.md` を参照（オンデマンド）。

## Key Colors

| Role | HEX |
|------|-----|
| Stripe Purple (CTA) | `#533afd` |
| Purple Hover | `#4434d4` |
...
| Brand Dark (sections) | `#1c1e54` |
| Success Green | `#15be53` |
| Ruby (decorative) | `#ea2261` |
| Magenta (decorative) | `#f96bee` |
| Shadow Blue | `rgba(50,50,93,0.25)` |

## Typography Essentials

- **Primary**: `sohne-var` (fallback: SF Pro Display) — OpenType `"ss01"` on all text; `"tnum"` for financial/tabular numbers
- **Monospace**: `SourceCodePro` (fallback: SFMono-Regular)
- **Weights**: 300 (headlines & body signature), 400 (buttons/links/nav); no 600–700 on sohne-var headlines
- **Display tracking**: -1.4px at 56px, -0.96px at 48px, progressive relaxation downward
