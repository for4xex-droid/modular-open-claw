...
**トークン節約**: この収集は機械的作業のため、低トークンのサブエージェントに委譲して構いません（本体は Phase 2 の評価に集中する）。
**前回比の準備**: 評価前に既存の `biz_value_report.html` を読み込み、前回の 7 軸スコアと総合スコアを抽出しておくこと。

> 出典: 2026-07-06 /biz-value 実行。メトリクス収集をサブエージェントに委譲し、前回レポート (2026-07-05, 総合3.1) からスコアを抽出して前回比つき評価（→3.4）を完成させたパターン。
