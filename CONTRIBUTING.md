...
- `/// # Safety` コメントは、実装者が unsafe の前提条件を理解していることを示す
  **自分の言葉で**記述すること。テンプレート的な safety コメントは拒否される。
- AI エージェントが GitHub Issue/PR にコメントする場合、先頭に `🤖 [Aiome Agent]` を付与する。
- Discord のサポート応答には末尾に `※ この回答は AI エージェントによる自動応答です` を付与する。
- 自律的に作成された PR には `ai-generated` ラベルを自動付与する。
- AI 支援を受けた PR は、PR テンプレートの「AI 支援の有無」チェックボックスにチェックを入れること。
