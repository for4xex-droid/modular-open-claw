...
async fn test_auth_admin_hash_fetch_failure_logging() {
    ...
    // system_settings テーブルをドロップしてDBエラーを発生させる。
    // 修正後の auth.rs では、get_setting_value() の Err 時に warn! ログを出力してから
    // None にフォールバックし、.env シークレットでの照合へ移行する。
    // テスト環境のシークレットと一致しないため最終的に FORBIDDEN となる。
    let payload = json!({
        "grant_type": "password",
