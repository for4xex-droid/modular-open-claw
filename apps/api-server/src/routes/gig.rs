...
pub async fn publish_intent(
    ...
    intent.description =
        shared::guardrails::strip_invisible_unicode(&intent.description).into_owned();
    ...
    for cri in &mut intent.criteria {
        match cri {
            AcceptanceCriteria::OracleJudge { rubric_prompt, .. } => {
                *rubric_prompt =
                    shared::guardrails::strip_invisible_unicode(rubric_prompt).into_owned();
            }
            ...
            AcceptanceCriteria::WasmValidator { wasm_module_cid } => {
                *wasm_module_cid =
                    shared::guardrails::strip_invisible_unicode(wasm_module_cid).into_owned();
            }
            AcceptanceCriteria::JsonSchema { schema } => {
                if let Ok(schema_str) = serde_json::to_string(schema) {
                    let clean =
                        shared::guardrails::strip_invisible_unicode(&schema_str).into_owned();
...
pub async fn deliver(
    ...
    deliverable.artifact_path =
        shared::guardrails::strip_invisible_unicode(&deliverable.artifact_path).into_owned();
