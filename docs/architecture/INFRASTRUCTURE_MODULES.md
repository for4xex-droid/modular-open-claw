...
---
*最終更新: 2026-07-10 (Asia/Tokyo) — OP-060 CoinChargeDlq / OP-061 OXP+RTBF / OP-067 htmd*
...
### Coin-charge DLQ Worker (OP-060)
- `api-server` の `internal_services::coin_charge_dlq_worker` が `SupervisedTask` として起動する。`outbox_dead_letters` の `coin_charge_failed` を起動直後＋60 秒周期で再送（`attempt_coin_charge_once`、OXP fail-closed）。成功時 DELETE、不正 JSON は `coin_charge_failed_poison` に隔離、再 INSERT 禁止。`CancellationToken` でシャットダウン。

### OXP / RTBF S2S (OP-061)
- Nurture 向け OXP は `OxiLeanProofCertificate::generate_header` に統一。stripe は `require_oxp_header()` fail-closed。アカウント削除 forget は `NURTURE_INTERNAL_SECRET` で OXP+Bearer、URL は `state.nurture_url`（`NURTURE_API_URL`）。
