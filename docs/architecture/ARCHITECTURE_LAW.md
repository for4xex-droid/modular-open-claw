...
本ドキュメントは、Aiome プロジェクトにおいて AI エージェント（アクター）を「自律的な部品」として安全かつ堅牢に運用するための基本法を定義する。
...
2.  **Abyss Vault (Key Isolation)**: API キー等の超機密情報はメインプロセスに保持してはならず、必ず別プロセスである `key-proxy` (The Abyss Vault) に隔離し、`mlockall` などの物理メモリ保護下で運用しなければならない。
3.  **Immutable Audit (Hash Chains)**: システムの実行ログや Karma（教訓）は、SQLite 上で暗号学的ハッシュチェーンを用いて記録されなければならず、改ざんや欠落を防ぐ監査性を死守しなければならない。
4.  **Graceful Exit**: 終了信号を受信した際は、進行中の全ての `Jail` 内タスクを安全に完了または中断し、ゾンビプロセスや一時ファイルを残さずに退出しなければならない。
...
1.  **Single-Tenant Policy**: リソースを大量に消費するタスク（LLM, 生成AI, 大型メディア処理）は、同時に複数が実行されてはならない。
2.  **Concurrency Control**: 重負荷アクターは、実行前にグローバルなセマフォ（`Semaphore`）から実行権限を取得し、終了時に即座に解放しなければならない。
...
3.  **The Media Processor (Standards)**: 生成されたメディアの品質基準を統一し、全体の音圧や色彩空間を標準規格に準拠させなければならない。
...
4.  **Verification and Synthesis**: スキルの生出力は、最終的に「人格（SOUL）」を持つアクターによって検証・再構成（Synthesis）され、ユーザーに届けられなければならない。

---

## 第10条：集合知ネットワーク (The Karma Federation)

システムは単一ノードの孤立した学習に留まらず、クラスタ全体でリアルタイムに教訓と防衛策を共有する「集合知（Karma Federation）」を形成しなければならない。

1.  **The Auth Wall**: フェデレーション通信は、環境変数 `FEDERATION_SECRET` による厳格な Bearer 認証によって保護されなければならず、未認証ノードからの「毒入れ（Poisoning）」を物理的に遮断する。
2.  **The Idempotent Sync**: ノード間のデータ同期は、ネットワークの遅延や重複リクエストに耐えうるよう、DBレイヤーでの `ON CONFLICT (id) DO NOTHING` 等を用いた完全な冪等性（Idempotency）を担保しなければならない。
3.  **Full Karma Coverage**: 共有される知識は、単なるテキストの教訓（Karma）に留まらず、自己防衛のための `ImmuneRules`（免疫ルール）および `ArenaMatch`（闘技場の勝敗録）を含有し、一つのノードの『発見』が瞬時に全ノードの『進化（または防衛）』となるよう設計されなければならない。

---

## 第11条：資産検疫とコンプライアンス (Asset Quarantine & Compliance)

システムは、外部から投入されるアセット（アバター、ボイスモデル等）に対し、法的・倫理的基準を自動的に適用し、ネットワーク全体の健全性を死守しなければならない。

1.  **eKYC Gate**: アセットの公開・共有を行うアクター（ユーザー）は、Stripe Identity 等を用いた本人確認および年齢確認を通過していなければならない。確認状態は `EkycSessionStore` によって永続化されなければならない。
2.  **Perceptual Triage (CSAM Defense)**: 全てのアセット（画像・音声）は、投入時に知覚ハッシュ（DCT等）による検閲を通過しなければならない。既知の違法コンテンツ（CSAM等）と一致した場合は、即座に破棄・隔離しなければならない。
3.  **Physical Integrity Rules**: アバターの 3D/2D モデルは、NURTURE 法規に基づく身体的プロポーション（5.5頭身等）の制約を遵守しなければならない。
4.  **The Quarantine Store**: 基準に満たない、または未検証のアセットは、決して本番環境やフェデレーション・ネットワークに流出させてはならず、必ず `QuarantineStore` (SQLite) に物理的に隔離し、監査可能な状態で保持しなければならない。
