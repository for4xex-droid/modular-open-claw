...
| 50 | **Infrastructure Fail-Open** | **DB failure treated as "setting not found"**| 🔴 High | **Result<?> Error Propagation (Fail-Closed) (RT1-1)**; **OP-075 / OP-075-B (2026-07-10/11)**: Immune Err Fail-Closed on chat (`evaluate_security`) **and** napi / goal_processor / nurture MCP / skill_handler |
...
- **Boundary Tautology Verification (Phase 47)**: Implements `BoundaryVerifier` as a microsecond-latency O(1) filter. It enforces immutable security invariants (shell meta-chars, restricted system paths, size limits) before any command reaches the OS shell, independent of LLM reasoning. Standard execution binaries like `podman` are natively whitelisted to ensure frictionless execution of rootless containers.
...
- **Vault Proxy Pattern (Secrets Brokering)**: `DockerConductor` and `BrowserConductor` permanently remove third-party billing keys (e.g., `GEMINI_API_KEY`) from their execution contexts. Instead, they dynamically broker an ephemeral internal `VAULT_SECRET` and `KEY_PROXY_URL` via `.env.shadow`. This ensures that even if an ephemeral container is compromised, the attacker can only access a scoped internal proxy endpoint, never the physical billing credentials.
...
- **PDF Extraction Process Isolation (pdftotext Migration)**: Replaces the vulnerable in-app `pdf-extract` library dependency with an isolated process-based parser using `pdftotext` (poppler). The binary is executed within the `SandboxProfile::Strict` environment (enforcing read-only root, no network, and no filesystem write) brokered via `SafeCommandBuilder`. Additionally, it enforces a 30-second execution timeout with strict child process reclamation (`kill` + `wait` to prevent zombie processes/PID leak) and buffer size caps (max 10MB stdout, 64KB stderr) to shield the host system from potential library exploits (RUSTSEC-2026-0187) or OOM/DoS attacks.
...
- **Sovereign Verification Pipeline (OxiLean)**: Integrating `oxilean-kernel` as the ultimate mathematical boundary for constitutional adherence. Proofs generated by the agent must pass the `grpc_proof_gate` without consuming excessive resources. The validation is deterministic, isolated from LLM drift, and acts as the highest tier of the Zero-Trust model.
- **CI Enforcement & Quality Gates**: The infrastructure enforces stringent CI policies through a matrix of gates (`pattern-enforcer.sh`, `docs-sync-check.sh`, `deep-scan.sh`). Anti-patterns such as `unwrap` and `expect` are rigorously hunted down to ensure memory and panic safety.
- **Test Flexibility Policy**: While production code maintains strict anti-panic guarantees, test modules are exempt via `#![allow(clippy::unwrap_used)]` or `// allow-anti-pattern`. This preserves developer velocity and testing simplicity without compromising the production core.
