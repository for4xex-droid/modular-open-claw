...
* **WASM Skills (軽量・高速)**
  ...
  * **厳格なホスト許可制 (OP-096 / ADR-057)**: 通信先はスキル Manifest の `allowed_domains` で明示許可が必要です。空リストは Deny（Fail-Closed）。判定は純関数 `host_permitted`（lowercase / FQDN 末尾ドット正規化後の完全一致 / サブドメイン / `*`。制御文字・内部空白は拒否。Bare TLD のサフィックス拡大は拒否）。Bastion の target 解決も Fail-Closed（`http`/`https`/`ws`/`wss` 以外および敵対的 bare host は deny）。Code Mode・制約チェッカーが同一関数を共有します。Extism への `*` 登録は行いません。
  ...
  * **seatbelt との役割分担 (OP-098)**: macOS `sandbox-exec` はネットワークの粗粒度（許可/拒否・LoRA 時の 80/443）のみ。Manifest ドメインを seatbelt に写経すると偽安心になるため実装しません。ホスト許可の正本は上記 `host_permitted` 層です。
