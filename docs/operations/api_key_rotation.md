...
### B. 管理コンソール（GUI）から更新する場合（**本番 compose 推奨**）
非技術者のユーザーや簡易的な管理を行いたい場合は、管理コンソール（GUI）から安全にシークレットを管理することができます。**本番 Docker ではホスト CLI よりこちらを優先**してください（key-proxy 経由で同一 Vault DB に書き込みます）。

1. **管理コンソールを開く**:
   「設定」ページ（SettingsPage）を開きます。
2. **Abyss Vault Secrets Manager セクション**:
   画面の下部にある「Abyss Vault シークレットマネージャ」セクションに移動します（このセクションは `viewMode` が `simple` であっても常時表示されます）。
3. **キーの設定・更新**:
