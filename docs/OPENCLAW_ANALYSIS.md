...
1. **最小権限の原則 (PoLP)**: 新しいツールを実装する際は、そのツールが必要とする最小限の URL やパスのみを `SecurityPolicy` に追加してください。
2. **記憶の保護**: API キーや個人情報は決して `MEMORY.md` に書かず、環境変数またはセキュアな秘密管理システムを使用してください。
3. **人間による承認 (Human-in-the-loop)**: 破壊的な操作（ファイルの削除や外部への投稿）を行うツールは、実行前に必ずユーザーの承認（通知・確認）を介するワークフローにしてください。

---

本ドキュメントの内容に基づき、安全かつ効率的な自動化工場の運用を継続してください。
