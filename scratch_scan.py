...
pattern = re.compile(r'(TODO|FIXME|GAP)', re.IGNORECASE)
                ...
                with open(path, 'r') as fp:
                    for i, line in enumerate(fp):
                        if pattern.search(line):
                            ...
                            if 'perfect' in path.lower() or 'roadmap' in path.lower() or 'memory' in path.lower() or 'changelog' in path.lower() or 'scratch' in path.lower() or 'full_todo' in path.lower():
                                continue
                            results.append(f"{path}:{i+1}: {clean_line}")
            except Exception:
                pass

with open('full_todo_scan.txt', 'w') as out:
    for r in results:
        out.write(r + '\n')
