...
- **`.context/RIPPLE_MAP.md`**: 各モジュールの依存関係と頻出カスケードパターンの影響範囲。コード変更前に必ずここを確認すること。
- **`docs/decisions/` (ADR)**: アーキテクチャやライブラリ選定、しきい値の論理的根拠など、変更してはならないルール。
- **`EVOLVING_SOUL.md`**: 過去の教訓（Karma）と、AIとしての最新の感情・状態。

この読込をスキップすることは、プロジェクトのコンテキスト崩壊・カスケードエラーを招くため、最優先事項として扱うこと。
