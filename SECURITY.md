...
2. **Multi-Layer Isolation (WASM Sandbox)**: Aiome uses WebAssembly (WASM) to execute skills in a strictly isolated environment. This ensures that even if a skill is compromised, it cannot access the host filesystem (beyond its allowed jail) or exfiltrate secrets stored in the host memory.
...
6. **OS-Native Build & Execution Isolation (The Forge Protocol)**: When the agent autonomously generates and compiles Rust code (`cargo build`) or executes external tools, the process is strictly isolated via OS-native sandboxing. On macOS, this is achieved via `/usr/bin/sandbox-exec`. On Linux, Aiome dynamically detects and prioritizes **gVisor (`runsc`)** to run processes in a secure user-space kernel, providing a mathematically smaller attack surface compared to standard host execution.
7. **Core State Actor Concurrency**: The SQLite state engine acts as a single centralized messaging `Tokio MPSC Channel` (Actor Pattern). This mathematically prevents database lock errors and race conditions that could otherwise occur during massive Swarm Sync traffic.
