...
9. **GDPR/RTBF & Content Compliance**: Guarantees complete physical data purge (via `forget_actor`) across up to 7 tables in a single atomic transaction, coupled with Zero-Trust downstream deletion to Nurture (`NURTURE_INTERNAL_SECRET` OXP+Bearer, canonical `NURTURE_API_URL`). Additionally, integrates an automated safety filter to detect and filter harmful content.
